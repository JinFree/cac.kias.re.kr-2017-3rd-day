...
void ParaWriter(int checker, float *U, float *V, float *W, float *Psi, int ReN, float Lx, float Ly, float delta, float dt, int Nx, int Ny);
void TechWriter(int checker, float *U, float *V, float *W, float *Psi, int ReN, float Lx, float Ly, float delta, float dt, int Nx, int Ny);
...
void ComputeMain(int check)
{
    ...
    printf("\nComputation Time: %.3lfms\n", time_ms);
    ParaWriter(check, U, V, W, Psi, ReN, Lx, Ly, delta, dt, Nx, Ny);
    TechWriter(check, U, V, W, Psi, ReN, Lx, Ly, delta, dt, Nx, Ny);
    free(U);
    free(V);
    free(W);
    free(Psi);
...
{
    int iter=0, SIZE = sizeof(float)*Nx*Ny;
    float error=1.0;
    do{
        iter++;
...
void ParaWriter(int checker, float *U, float *V, float *W, float *Psi, int ReN, float Lx, float Ly, float delta, float dt, int Nx, int Ny)
{
    ...
    char name[150] = "Error";
    if(checker==1)
        sprintf(name, "CPU, Re=%d, delta=%f, Lx=%.1f, Ly=%.1f, dt=%f.csv", ReN, delta, Lx, Ly, dt);
    else if(checker==2)
        sprintf(name, "GPU, Re=%d, delta=%f, Lx=%.1f, Ly=%.1f, dt=%f.csv", ReN, delta, Lx, Ly, dt);
    PR = fopen(name, "w");
    int i,j;
...
void TechWriter(int checker, float *U, float *V, float *W, float *Psi, int ReN, float Lx, float Ly, float delta, float dt, int Nx, int Ny)
{
	...
	char name[150] = "Error";
	if(checker==1)
        sprintf(name, "CPU, Re=%d, delta=%f, Lx=%.1f, Ly=%.1f, dt=%f.dat", ReN, delta, Lx, Ly, dt);
	else if(checker==2)
        sprintf(name, "GPU, Re=%d, delta=%f, Lx=%.1f, Ly=%.1f, dt=%f.dat", ReN, delta, Lx, Ly, dt);
    TP = fopen(name, "w");
	int i, j;
